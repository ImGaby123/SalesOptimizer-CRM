import sys
from PySide6.QtWidgets import QApplication, QMainWindow, QLineEdit, QHeaderView, QPushButton
from PySide6.QtGui import QIcon
from views.py.ui_contacts import Ui_contacts  # Use Ui_Form if that's the generated class name

class MainWindow(QMainWindow, Ui_contacts):
    def __init__(self):
        super().__init__()
        self.setupUi(self)

        # Initialize current index for stacked widget
        self.current_page_index = 1

        self.tableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.tableWidget_2.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

        # Connect navigation buttons
        self.btn_next = QPushButton("Next", self)
        self.btn_previous = QPushButton("Previous", self)

        # Set button positions (you can modify these as needed)
        self.btn_next.setGeometry(1100, 630, 80, 30)
        self.btn_previous.setGeometry(1000, 630, 80, 30)

        # Connect buttons to functions
        self.btn_next.clicked.connect(self.go_to_next_page)
        self.btn_previous.clicked.connect(self.go_to_previous_page)

        # Set initial page in stacked widget
        self.stackedWidget.setCurrentIndex(self.current_page_index)

        # Add the icon to the QLineEdits programmatically
        self.add_icon_to_lineedit(self.search, ":/new/newPrefix/Resources/magnifying-glass.png")
        self.add_icon_to_lineedit(self.search_2, ":/new/newPrefix/Resources/magnifying-glass.png")

    def add_icon_to_lineedit(self, lineedit, icon_path):
        """Set the icon inside the QLineEdit and resize it."""
        # Create a QIcon with the given icon path
        icon = QIcon(icon_path)

        # Use QLineEdit's addAction to add the icon to the leading (left) side of the QLineEdit
        lineedit.addAction(icon, QLineEdit.ActionPosition.LeadingPosition)

        # Resize the icon itself if needed
        lineedit.setStyleSheet(f"""
            QLineEdit {{
                border: 2px solid black;
                border-radius: 10px;
                padding-left: 0px;  /* Space for the icon */
                }}
            QLineEdit::indicator {{
                width: 20px;  /* Icon width */
                height: 20px;  /* Icon height */
            }}
        """)

    def go_to_next_page(self):
        # Get the total number of pages in the stacked widget
        total_pages = self.stackedWidget.count()

        # Move to next page and wrap around
        self.current_page_index = (self.current_page_index + 1) % total_pages
        self.stackedWidget.setCurrentIndex(self.current_page_index)

    def go_to_previous_page(self):
        # Get the total number of pages in the stacked widget
        total_pages = self.stackedWidget.count()

        # Move to previous page and wrap around
        self.current_page_index = (self.current_page_index - 1) % total_pages
        self.stackedWidget.setCurrentIndex(self.current_page_index)

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = MainWindow()
    window.show()
    sys.exit(app.exec())
