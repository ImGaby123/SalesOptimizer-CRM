...
class CustomMdiSubWindow(QMdiSubWindow):
    def __init__(self):
        ...
        # Set window title
        self.setWindowTitle("Custom MDI Window")

        # Apply a workaround to change the title bar color
        self.setStyleSheet("""
            QMdiSubWindow {
            ...
            QMdiSubWindow QWidget {
                background-color: #333;  /* Title bar color */
                color: white;
                font: bold 12px Arial;
            }
        """)

        # Add a simple widget inside the subwindow
        widget = QWidget()
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Hello, MDI!"))
